...
public static int save(StudentBean bean){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into fee_student(rollno,name,sex,course,fee,paid,due,address,contact,first_inst,second_inst,third_inst,mname,lname) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		ps.setInt(10, bean.getF_inst());
		ps.setInt(11, bean.getS_inst());
		ps.setInt(12, bean.getT_inst());
		ps.setString(13, bean.getMname());
		ps.setString(14, bean.getLname());
...
public static List<StudentBean> getAllRecords(){
	...
	try{
		Connection con=DB.getCon();
		PreparedStatement ps=con.prepareStatement("select rollno,name,sex,course,fee,paid,due,address,contact from fee_student");
...
public static StudentBean getRecordByRollno(int rollno){
	...
	try{
		...
		while(rs.next()){
			...
			bean.setAddress(rs.getString(8));
			bean.setContact(rs.getString(9));
			bean.setF_inst(rs.getInt(10));
			bean.setS_inst(rs.getInt(11));
			bean.setT_inst(rs.getInt(12));
